...
let rec state_str_from_fml fml = 
	match fml with
	| Top
	| Bottom
	| Atomic (_, _)
	...
	| And (_, _)
	| Or (_, _) -> "Nothing"
	| AX (s, fml1, s') -> (str_state s')
	| EX (s, fml1, s') -> (str_state s')
	| AF (s, fml1, s') -> (str_state s')
	| EG (s, fml1, s') -> (str_state s')
	| AR (s, s', fml1, fml2, s'') -> (str_state s'')
	| EU (s, s', fml1, fml2, s'') -> (str_state s'')
